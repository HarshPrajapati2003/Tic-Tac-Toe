console.log("Tic Tac Toe");
let music = new Audio("music.mp3")
let AudioTurn = new Audio("ting.mp3")
let gameover = new Audio("over.mp3")
let winning = new Audio("Winning.mp3")
let isOver = false;
let turn="X";

// Function to change turn
const changeTurn = () =>{
    return turn==="X" ? "0":"X";
}

// Function to check for the win
const checkWin = () =>{
    let boxtext=document.getElementsByClassName("boxtext");
    let wins = [
        [0,1,2,0,5,0],
        [3,4,5,0,15,0],
        [6,7,8,0,25,0],
        [0,3,6,-10,15,90],
        [1,4,7,0,15,90],
        [2,5,8,10,15,90],
        [0,4,8,0,15,45],
        [2,4,6,0,15,136]
    ];
    wins.forEach(e=>{
        if((boxtext[e[0]].innerHTML === boxtext[e[1]].innerHTML) && (boxtext[e[2]].innerHTML === boxtext[e[1]].innerHTML) && (boxtext[e[0]].innerHTML !== "")){
            document.getElementsByClassName("info")[0].innerHTML = boxtext[e[0]].innerHTML + " Won Match";
            isOver = true; 
            document.querySelector(".imgbox").getElementsByTagName("img")[0].style.height="40vh"
            document.getElementsByClassName("info")[0].style.color="green";
           
            document.querySelector(".line").style.transform =`translate(${e[3]}vw, ${e[4]}vw) rotate(${e[5]}deg)`;
            document.querySelector(".line").style.width = "30vw"
            winning.play();
        }
    })
}


// Game Logic 
music.play();
let boxes = document.getElementsByClassName("box");
Array.from(boxes).forEach(element=>{
    let boxtext = element.querySelector(".boxtext");
    element.addEventListener('click',()=>{
        if(boxtext.innerHTML===''){
            boxtext.innerHTML=turn;
            turn = changeTurn();
            AudioTurn.play();
            checkWin();
            if(isOver != true){
                document.getElementsByClassName("info")[0].innerHTML = "Turn for "+ turn;
            }
         
        }
    })
})

// add onclick listener to reset button
reset.addEventListener('click',()=>{
    let btexts = document.querySelectorAll(".boxtext");
    Array.from(btexts).forEach(ele =>{
        ele.innerHTML="";
      
    })
    turn = "X";
    isOver = false;
    document.getElementsByClassName("info")[0].innerHTML = "Turn for "+ turn;
    document.querySelector(".imgbox").getElementsByTagName("img")[0].style.height="0vh"
    document.querySelector(".line").style.width = "0vw"
})